...
if __name__ == "__main__":
    path = "D:\Desktop\\research\SEED_IV\eeg_feature_smooth"
    # seed = SEEDFeatureDataset(root_path = path, channels=["FP1", "F5", "F8"])
    seed = SEEDIVFeatureDataset(root_path = path, subjects=[1], session=[1]).data()
    print(seed[0].shape, seed[1].shape, seed[2].shape)
    # print(seed[1])
    print(np.unique(seed[2][:, 1])) #subject
    print((seed[2][:, 2])) #trial