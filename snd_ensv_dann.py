...
def score(args, model, feature, labels):
    ...
    with torch.no_grad():
        ...
        normalized = F.normalize(all_output).cpu()  # 正则化
        mat = torch.matmul(normalized, normalized.t()) / 0.05  # 温度系数
        mask = torch.eye(mat.size(0), mat.size(0)).bool()  # 对角线掩码
        mat.masked_fill_(mask, -1 / 0.05)  # 将对角线设为负无穷
        ent_soft = entropy(mat)

        # 计算类别预测的熵
        ent_class = ent(all_output)
...
def val(args, model_list, is_five=True):
    ...
    for model_path in model_list:
        model = Base(num_of_class=args.class_num)
    ...
    while True:  # 持续循环，直到不需要排除为止
        ...
        for model_path in model_list:
            ...
            # print(f"acc:{acc}, pred:{pred}")
            score_list.append(ent_soft)#SND
        ...
        # sorted_indices = torch.argsort(ac_list, descending=True)
        # print(sorted_indices)
        SND_WEIGHT = 0.2 # SND的权重
        ENT_WEIGHT = 0.8 #Entropy的权重
        ENS_WEIGHT = 1-SND_WEIGHT-ENT_WEIGHT # Ensemble的权重
        final_list = SND_WEIGHT * score_list + ENS_WEIGHT * ac_list - ENT_WEIGHT * ent_list # 加权平均
        # 检查是否需要排除最差模型
        if max(ac_list) - min(ac_list) > args.flu_level:
            # 排除最差模型
            ...
            print()
            best_model = model_list[torch.argmax(final_list)]#SND_ENSV
            ...
            if is_five == True:
                best_model = best_model.split("_weight")[1].split("\\")[0]
            else:
                best_model = best_model.split("_to")[0].split("\\")[-1]  # 提取模型名称,针对一对一的情况
            # # 获取ac_list第二高的模型
            # best_model_v = model_list[sorted_indices[0]]
            # best_model_v = best_model_v.split("_to")[0].split("\\")[-1]
            # print(f"Best model: {best_model_v}")
            # second_best_model = model_list[sorted_indices[1]]
            # second_best_model = second_best_model.split("_to")[0].split("\\")[-1] 
            # print(f"Second highest model: {second_best_model}")
            print(f"acc: {best_acc*100:.2f}")
            print(f"After excluding, snd and entropy target{args.target}'s best model is : {best_model}")
            break  # 退出循环
        
def single_val2(args):# 计算一次，排除一次
    ...
    if args.dset == 'seed':
        for source in range(1, 16):  # 每个目标域14个源域分别训练
            if args.target == source:
                continue
            model_list.append(f".\\logs\\logs_base_dann\\session_1\\source{source}_to_target{args.target}\\best.pth")
    else:
        for source in range(1, 16):  # 每个目标域14个源域分别训练
            if args.target == source:
                continue
            model_list.append(f".\\logs\\logs_base_dann_iv\\session_2\\source{source}_to_target{args.target}\\best.pth")
...
if __name__ == "__main__":
    ...
    if args.dset == 'seed':
        ...
        args.class_num = 3
        model_list = [f".\\logs\\logs_base_dann_weight0.01\\session_1\\{args.target}\\best.pth",
                f".\\logs\\logs_base_dann_weight0.1\\session_1\\{args.target}\\best.pth",
                f".\\logs\\logs_base_dann_weight0.5\\session_1\\{args.target}\\best.pth",
                f".\\logs\\logs_base_dann_weight1\\session_1\\{args.target}\\best.pth",
                f".\\logs\\logs_base_dann_weight2\\session_1\\{args.target}\\best.pth"]
    
    if args.dset == 'seediv':
        args.path = "D:\\Desktop\\research\\SEED_IV\\eeg_feature_smooth\\"
        args.class_num = 4
        model_list = [f".\\logs\\logs_base_dann_iv_weight0.01\\session_2\\{args.target}\\best.pth",
                  f".\\logs\\logs_base_dann_iv_weight0.1\\session_2\\{args.target}\\best.pth",
                  f".\\logs\\logs_base_dann_iv_weight0.5\\session_2\\{args.target}\\best.pth",
                  f".\\logs\\logs_base_dann_iv_weight1\\session_2\\{args.target}\\best.pth",
                  f".\\logs\\logs_base_dann_iv_weight2\\session_2\\{args.target}\\best.pth"]
