...
def load_seed(args, target, source_lists = None):
    ...
    one_hot_mat = np.eye(len(Label), args.num_of_class)[Label].astype("float32")
     ...
     # 源域数据，如果没有指定源域，则是其余14个受试者一起作为源域
    ...
    # 获得源域数据
    source_features = torch.from_numpy(EEG[np.isin(Group, source_lists)]).type(torch.Tensor)
    source_labels = torch.from_numpy(one_hot_mat[np.isin(Group, source_lists)])
...
def train(args, target, source_lists=None):
...
def main(args):
    ...
    # 4: [2, 5, 9, 12, 14],
    # 5: [3, 4, 6, 8],
    # 6: [2, 3, 5, 9, 10, 11, 14, 15],
    # 7: [2, 11, 15],
    # 8: [4, 6],
    # 9: [1, 4, 8, 12],
    # 10: [1, 3, 9, 12],
    # 11: [2, 12],
    # 12: [1, 4, 5, 8, 10, 13, 14, 15],
    # 13: [2, 5, 8],
    # 14: [2, 10, 13],
    # 15: [3, 4, 5, 13],
    # }
    ...
    for target in range(1, 16):
        best_acc = train(args, target, source_lists=None)
    ...
    for target, best_acc in enumerate(best_acc_mat):
        print(f"target: {target+1}, best_acc: {best_acc:.2f}")
    print(f"all_best_acc: {mean:.2f} ± {std:.2f}")
